...
pageMod.PageMod({
    ...
    onAttach: function(worker) {
        ...
        worker.port.on("links", function(payload) {
            ...
            var payload = payload.filter(function(pageURL) {
            ...
            payload.filter(isCached).forEach(function(pageURL) {
                getPageInfo(pageURL, function(data) {
                    if (data != null && shouldBeColored(data)) {
                        worker.port.emit("color", data, storage.properties);
                    }
                });
            });
            ...
            notCachedPages.forEach(function(pageURL) {console.log("main: not cached url = " + pageURL)});

            var analyzedLinks = 0;

            notCachedPages.forEach(function(pageURL, index) {
                function getSuccessFunc(callback) {
                    return function success(data) {
                        if (data != null && shouldBeColored(data)) {
                            worker.port.emit("color", data, storage.properties);
                        }
                        ++analyzedLinks;
                        worker.port.emit("shiftProgressBar", (analyzedLinks / notCachedPages.length));

                        if (callback !== undefined && data != null) {
                            callback(data);
                        }
                    }
                }

                ajaxModule.getInfoFromServer(pageURL, getSuccessFunc(), function(pageURL) {
                    require("sdk/timers").setTimeout(function() {
                        getPageInfo(pageURL, getSuccessFunc(ajaxModule.giveInfoToServer));
                    }, 5000 * index);
                });
            });
        });
...
function getPageInfo(pageURL, callback) {
    if (!storage.cache.hasOwnProperty(pageURL)) {
        parserModule.downloadAndparse(pageURL, function(data) {
            storage.cache[pageURL] = data;
            callback(data);
        });
    } else {
        callback(storage.cache[pageURL]);
    }
}
