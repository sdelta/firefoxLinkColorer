...
function getPageInfo(pageURL, destination) {
    console.log("main: starting getPageInfo function with page = " + pageURL);

    var worker = require("sdk/page-worker").Page({
        contentScriptFile: [data.url("jquery-2.1.1.js"), data.url("analyzeSamlibPage.js")],
        contentURL: "file:///home/delta/Downloads/test.shtml" //pageURL
    });

    console.log("main: from main analyzer started");

    worker.port.once("analyzed", function(payload) {
        console.log("main: received message from analyzer");
        worker.destroy();
        storage.cache[payload.url] = payload;
        if (shouldBeColored(payload)) {
            console.log("main: repeat message from analyzer");
            destination.port.emit("color", payload);
        }
    });
}
...
var propertiesPanel = require("sdk/panel").Panel({
    ...
    contentScriptFile: [data.url("jquery-2.1.1.js"), data.url("propertiesScript.js")]
});
...
var buttonPopup = buttons.ActionButton({
    ...
    icon: {
        "16": "./icon-properties-16.png",
        "32": "./icon-properties-32.png",
        "64": "./icon-properties-64.png"
    },
...
var buttonClearCache = buttons.ActionButton({
    ...
    label: "clear cache",
    icon: {
        "16": "./icon-clear-16.png",
        "32": "./icon-clear-32.png",
        "64": "./icon-clear-64.png"
    },
    onClick: function() {
        storage.cache = {};
    }
...
function shouldBeColored(elem) {
    console.log("main: in shouldBeColored");
    console.log("main: bookPage = " + storage.properties.bookPageMinSize);
    console.log("main: authorPage = " + storage.properties.authorPageMinSize);
    ...
    return isGoodAuthor || isGoodBook;
}
