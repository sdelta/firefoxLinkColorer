...
pageMod.PageMod({
    include: "*",
    contentScriptFile: [data.url("../lib/parser.js"), data.url("nanobar.min.js"), data.url('highlightLinks.js')],
    attachTo: "top",
        ...
        worker.port.on("links", function(payload) {
            ...
            for (var i = 0; i < payload.length; ++i) {
                function callback(data) {
                    if (shouldBeColored(data)) {
                        worker.port.emit("color", data, storage.properties);
                    }
                }

                function callAnyway() {
                    worker.port.emit("linkAnalyzed");
                }

                getPageInfo(payload[i], callback, callAnyway);
            }
        });
    }
...
function getPageInfo(pageURL, callback, callAnyway) {
